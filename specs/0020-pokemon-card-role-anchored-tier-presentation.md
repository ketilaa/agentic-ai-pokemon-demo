...
The developer must remove the `RoleTierSection` and `RoleMovesetSection` sub-components and replace them with a unified sub-component implementing the `pve-roles-section`. Tests covering the retired elements must be removed and replaced with tests for the new structure:

- **Spec 0018 component tests to remove:** AC-15, AC-16, AC-17, AC-19, AC-20, the placement test (`'role-tier-section is inside card-content-section'`), and the ordering test (`'role-tier-section appears after stat-bar-sta and before move-section in document order'`). All seven query `role-tier-section` or retired attributes (`data-type-id`) and will fail or become incorrect after implementation.
- **Spec 0019 component tests to remove:** AC-01 through AC-19 in full — all nineteen. Eight of them (AC-04, AC-05, AC-06, AC-09, AC-10, AC-11, AC-12, AC-15) query `role-moveset-section` or `role-moveset-group` directly and will throw with element-not-found failures. The remaining eleven (including AC-07 and AC-08) will continue to pass after implementation — AC-07 and AC-08 pass because they are scoped to `within(move-section)` and `pve-roles-section` is outside `move-section` — but they represent an obsolete design intent and must be removed, not left in place as silent false positives.
...
- **Large test surface requiring updates.** This spec retires `role-tier-section`, `role-moveset-section`, and `role-moveset-group` and supersedes behavior from both spec 0018 and spec 0019. The developer must remove: spec 0018 component tests AC-15, AC-16, AC-17, AC-19, AC-20, plus the two unnamed structural tests (placement test `'role-tier-section is inside card-content-section'` and ordering test `'role-tier-section appears after stat-bar-sta...'`); spec 0019 component tests AC-01 through AC-19 in full. Eight spec 0019 tests (AC-04–AC-06, AC-09–AC-12, AC-15) query retired elements and will throw; the other eleven will pass silently but represent obsolete behavior and must be removed. The developer must audit the full test file before beginning implementation, not rely solely on test failures to find affected tests.

- **Single-role filter change — spec 0019 AC-18/AC-19 will fail; AC-07/AC-08 will pass but are obsolete.** AC-18 and AC-19 assert that a single-role Pokémon's `quick-moves-group` and `charged-moves-group` contain the recommended move. Under spec 0020, recommended moves for single-role Pokémon are inside the attacker role block; the standard groups are filtered. AC-18 and AC-19 will fail and must be replaced with AC-19 and AC-20 from this spec. AC-07 and AC-08 assert that no role label appears within `move-section` for single-role and fallback Pokémon respectively; these will continue to pass because `pve-roles-section` is outside `move-section`. They must nonetheless be removed — they document the old "no label for single-role Pokémon" design intent, which this spec supersedes.
